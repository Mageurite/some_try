...
function UserTable({ themeStyles, selectedMenu, onLogout }) {
    ...
    return (
        <div style={{ position: 'relative', width: '100%', height: '100%' }}>
            ...
            {/* 右上角头像和退出按钮 */}
            <div style={{ position: 'absolute', top: 24, right: 40, zIndex: 20 }}>
                <div
                ...
                >
                    <img
                        src={avatarSrc}
                        alt=""
                        style={{ width: 48, height: 48, borderRadius: '50%', objectFit: 'cover', border: '2px solid #eee', boxShadow: '0 2px 8px #eee', cursor: 'pointer', background: '#f0f0f0' }}
            ...
            {/* 標題和工具欄 */}
            <div style={{ padding: '32px 32px 0 32px', background: themeStyles.toolbarBg, display: 'flex', flexDirection: 'column', gap: 16 }}>
                <div style={{ fontWeight: 700, fontSize: 28, marginBottom: 8 }}>{config.title}</div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
                    <div style={{ display: 'flex', gap: 12 }}>
                        <input
                            ...
                            style={{ padding: '8px 12px', borderRadius: 6, border: `1px solid ${themeStyles.tableBorder}`, fontSize: 15, outline: 'none', minWidth: 140 }}
                        />
                        <input
                            type="text"
                            ...
                            {selectedMenu === 'knowledge' ? (
                                <>
                                    <label htmlFor="file-upload" style={{
                                        display: 'inline-block',
                                        padding: '8px 16px',
                                        background: uploadingFile ? '#e0e0e0' : '#4CAF50',
                                        color: '#fff',
                                        borderRadius: 8,
                                ...
                                <Button
                                    variant="contained"
                                    style={{ background: themeStyles.createBtn, color: themeStyles.createBtnText, fontWeight: 700, borderRadius: 8, fontSize: 16, padding: '8px 28px' }}
            ...
            <div style={{ flex: 1, overflow: 'auto', padding: 32 }}>
                <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, background: themeStyles.background, borderRadius: 12, boxShadow: '0 2px 12px #0001' }}>
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        {pagedData.map((row, idx) => (
                            <tr
                                key={row.id}
                                style={{
                                    background: row.id === selectedRow ? '#FFF9E1' : 'transparent',
                                    transition: 'background 0.2s',
                                    cursor: 'pointer'
                                }}
                                            ...
                                            {selectedMenu === 'knowledge' ? (
                                                ...
                                                <>
                                                    {selectedMenu !== 'avatar' && selectedMenu !== 'logs' && (
                                                        <IconButton
                                                            style={{
                                                                background: themeStyles.editBtnBg(row.id === selectedRow),
                                                                color: themeStyles.editBtn(row.id === selectedRow),
                                                                borderRadius: 8
                                                            }}
                                                            onClick={(e) => {
                                                                e.stopPropagation();
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 24 }}>
                    ...
                    <div style={{ display: 'flex', gap: 4 }}>
                        {pageCount > 0 ? (
                            Array.from({ length: pageCount }, (_, i) => (
                                <button
                                    ...
                                    style={{
                                        ...
                                        fontSize: 15,
                                        cursor: 'pointer',
                                        boxShadow: page === i + 1 ? '0 2px 8px #FFD60033' : 'none',
                                        transition: 'all 0.2s'
                                    }}
                                >
                                    {i + 1}
                            ...
                            <div style={{ fontSize: 15, color: '#888' }}>No data available</div>
                        )}
                    </div>
                </div>
            </div>
